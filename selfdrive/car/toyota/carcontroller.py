...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    if not enabled or CS.steer_state in [9, 25] or ((abs(CS.out.steeringAngle) < 100 and abs(CS.out.steeringRate) > 100 and abs(CS.out.steeringTorque) < 200) and self.op_params.get('steer_fault_fix')):
      apply_steer = 0
      apply_steer_req = 0
    else:
      apply_steer_req = 1
