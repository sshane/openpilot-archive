...
class Traffic:
  def __init__(self, use_probability=False):
    ...
    self.past_preds = []
    self.model_rate = 1 / 5.
    self.recurrent_length = 2.0  # in seconds, how far back to factor into current prediction
    self.des_pred_len = int(self.recurrent_length / self.model_rate)
    self.last_pred_weight = 4.0  # places 2x weight on most recent prediction
