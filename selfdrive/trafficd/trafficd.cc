...
std::vector<float> processStreamBuffer(VIPCBuf* buf) {
    ...
    for (int line = 0; line < cropped_shape[0]; line++) {
        for(int line_pos = 0; line_pos < (cropped_shape[1] * cropped_shape[2]); line_pos += cropped_shape[2]) {
            outputVector.insert(0, src_ptr[line_pos + offset + 0] / pixel_norm);
            outputVector.insert(0, src_ptr[line_pos + offset + 1] / pixel_norm);
            outputVector.insert(0, src_ptr[line_pos + offset + 2] / pixel_norm);
        }
