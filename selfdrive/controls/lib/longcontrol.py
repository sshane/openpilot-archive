...
class LongControl():
  def __init__(self, CP, compute_gb):
    ...
    # self.past_data = []
    self.scales = {'v_ego': [0.0, 39.12937927246094], 'x_lead': [0.125, 138.625], 'v_lead': [0.0, 34.16797637939453],
                   'a_lead': [-6.480010509490967, 14.326614379882812]}
  ...
  def df_controller_v_future(self, v_ego, a_ego, track_data, steering_angle, steering_rate, left_blinker, right_blinker,
  ...
  def update(self, active, v_ego, a_ego, set_speed, radar_state, brake_pressed, standstill, cruise_standstill, v_cruise, v_target, v_target_future, a_target, CP, track_data, steering_angle,
    ...
    long_model_output = self.df_controller(v_ego, a_ego, track_data, steering_angle, steering_rate, left_blinker, right_blinker,
                                           radar_state, set_speed)
