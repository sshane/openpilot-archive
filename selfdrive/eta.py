...
class ETA:
  ...
  def set_ips(self):
    ...
    self.last_ips = (self.progress - self.last_progress) / (self.time - self.last_time)
  ...
  def get_eta(self):
    ...
    ips = self.total_ips * 0.8 + self.last_ips * 0.8
    if self.last_ips < self.total_ips:
      ips = self.last_ips * 0.8 + self.total_ips * 0.8

    if self.last_ips > 10:  # probably pulling from cache
