...
def data_sample(CI, CC, sm, can_poller, can_sock, cal_status, cal_perc, overtemp, free_space, low_battery,
  ...
  return CS, events, cal_status, cal_perc, overtemp, free_space, low_battery, mismatch_counter, last_radar_state
...
def state_control(frame, rcv_frame, plan, path_plan, radar_state, CS, CP, state, events, v_cruise_kph, v_cruise_kph_last,
                  AM, rk, driver_status, LaC, LoC, read_only, is_metric, cal_perc, liveTracks):
...
def controlsd_thread(sm=None, pm=None, can_sock=None):
  ...
  while True:
    ...
    CS, events, cal_status, cal_perc, overtemp, free_space, low_battery, mismatch_counter, last_radar_state =\
    ...
    actuators, v_cruise_kph, driver_status, v_acc, a_acc, lac_log = \
      state_control(sm.frame, sm.rcv_frame, sm['plan'], sm['pathPlan'], last_radar_state, CS, CP, state, events, v_cruise_kph, v_cruise_kph_last, AM, rk,
                    driver_status, LaC, LoC, read_only, is_metric, cal_perc, liveTracks)
