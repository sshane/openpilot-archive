...
# make sure these accelerations are smaller than mpc limits
_A_CRUISE_MAX_V = [2.5, 2.5, 1.5, .5, .3] # [1.1, 1.1, .8, .5, .3]
_A_CRUISE_MAX_V_FOLLOWING = [2.0, 1.8, 1.5, .7, .3] # [1.6, 1.6, 1.2, .7, .3]
...
class Planner(object):
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    self.mpc1.update(pm, sm['carState'], lead_1, track_data, sm['controlsState'], v_cruise_setpoint)
    self.mpc2.update(pm, sm['carState'], lead_2, track_data, sm['controlsState'], v_cruise_setpoint)
